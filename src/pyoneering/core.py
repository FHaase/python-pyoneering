...
def generate_version_identifiers(stage_names, version_identifier: List[str]):
    return list(map(lambda x: Stage(*x), zip(stage_names, map(version.parse, version_identifier))))


def validate_version_identifiers(stages: List[Stage]):
...
def create_stage_information(current_version: version.Version or version.LegacyVersion, stages: List[Stage]):
    next_stage = None
